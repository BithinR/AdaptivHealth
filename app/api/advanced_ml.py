...
recommendation ranking, natural language alerts, model retraining, and explainability.
"""
...
async def optimize_baseline(
    ...
    # Get resting-phase readings (or all if activity_phase not available)
    vitals = (
        db.query(VitalSignRecord)
        .filter(
            VitalSignRecord.user_id == current_user.user_id,
    ...
    # Filter for likely resting readings (HR < 100 as a heuristic)
    resting_readings = [
        {"heart_rate": v.heart_rate}
        for v in vitals
...
async def apply_baseline_optimization(
    ...
    if result.get("adjusted") and result.get("new_baseline"):
        ...
        result["applied"] = True
        logger.info(
            f"Baseline updated for user {current_user.user_id}: "
            f"{result.get('current_baseline')} -> {result['new_baseline']}"
        )
    else:
...
async def get_ranked_rec(
    ...
    which suggestions lead to better outcomes.
    """
...
async def model_retraining_status(
    ...
    """
    Get the current model status and retraining metadata.
...
async def check_retraining_readiness(
    ...
    """
    Check if conditions are met to retrain the model.

    Clinician/Admin access only.
    """
    # Count new records since model was last modified
    status = get_retraining_status()
    last_date = None
    if status.get("metadata"):
        last_date = status["metadata"].get("retrained_at")

    # Count total risk assessments (as proxy for new data)
