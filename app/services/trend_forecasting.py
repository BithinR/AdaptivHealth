...
def forecast_trends(
    ...
    Args:
        readings: Historical vital sign readings (at least 7 needed).
        forecast_days: Number of days to forecast ahead.

    Returns:
        Dict with trend direction, forecasted values, and risk projection.
    """
    ...
    # Forecast heart rate trend
    if len(hr_series) >= 7:
        hr_trend = _linear_forecast(hr_series, forecast_days)
        result["trends"]["heart_rate"] = hr_trend

    # Forecast SpO2 trend
    if len(spo2_series) >= 7:
        spo2_trend = _linear_forecast(spo2_series, forecast_days)
        result["trends"]["spo2"] = spo2_trend
...
def _linear_forecast(
    ...
    Args:
        series: List of (day_offset, value) tuples.
        forecast_days: Days to forecast into future.

    Returns:
        Dict with slope, direction, current/forecasted values, r_squared.
    """
    n = len(series)
    ...
    # Current fitted value (at last data point)
    last_day = xs[-1]
