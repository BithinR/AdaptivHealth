...
def evaluate_retraining_readiness(
    ...
    Args:
        new_records_count: Number of new data records since last training.
        last_retrain_date: ISO date string of last retraining.
        min_records: Minimum new records required.
        min_days_since_last: Minimum days since last retrain.

    Returns:
        Dict indicating readiness and reasons.
    """
    reasons = []
    ready = True
    ...
    if new_records_count < min_records:
        ready = False
        reasons.append(
            f"Only {new_records_count} new records (need {min_records})"
        )
...
def prepare_training_data(
    ...
    Args:
        records: List of dicts with vital sign and outcome data.

    Returns:
        Dict with prepared features, labels, and data quality report.
    """
    if not records:
        return {
            "status": "no_data",
...
def get_retraining_status() -> Dict[str, Any]:
    ...
    Returns:
        Dict with model file info, versions, and timestamps.
    """
    ...
    if model_path.exists():
        ...
        status["model_modified"] = datetime.fromtimestamp(
...
def save_retraining_metadata(
    ...
    Args:
        ...
        records_used: Number of records used in training.
    ...
    try:
        ...
        logger.info(f"Model metadata saved: version={version}, accuracy={accuracy}")
