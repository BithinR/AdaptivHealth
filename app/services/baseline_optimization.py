...
def compute_optimized_baseline(
    ...
    Args:
        resting_readings: List of dicts with 'heart_rate' from resting periods.
        current_baseline: Current stored baseline HR (or None if not set).
        smoothing_factor: Weight for new data (0.0=ignore new, 1.0=use only new).

    Returns:
        Dict with new_baseline, adjustment info, and confidence.
    """
    ...
    hr_values = [
        r["heart_rate"] for r in resting_readings
    ...
    # Filter outliers (beyond 1.5 std from mean)
    filtered = [v for v in hr_values if abs(v - mean_hr) <= 1.5 * std_hr] if std_hr > 0 else hr_values
    if len(filtered) < 3:
        filtered = hr_values

    filtered_mean = sum(filtered) / len(filtered)

    # Compute new baseline using exponential moving average
    if current_baseline is not None:
        new_baseline = int(round(
            current_baseline * (1 - smoothing_factor) + filtered_mean * smoothing_factor
        ))
    else:
